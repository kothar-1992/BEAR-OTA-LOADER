...
//			LOGE("完成=%s!!耗时=%lf毫秒 ，耗费na秒=%lld\n",name.c_str(), runtime/1000000.0,runtime);
	...
	// 使用 AutoFPS 之前 需调用 SetFps ，参数 单位 每秒循环次数
	...
	//初始化 AutoFPS ， 必须在 AutoFPS 之前调用
	inline void AutoFPS_init(bool a){
		clock_gettime(CLOCK_MONOTONIC,&old);
		SleepTime = Fps;
		start();
	}
	long long oldtimer;
	//调用 AutoFPS 之前 请调用 AutoFPS_init  和 SetFps， SetFps 可实时调整FPS。此函数建议放在循环最前面。函数返回值为当前循环时间，单位毫秒， 1000.0f/返回值可得到准确fps

	inline float AutoFPS(){
