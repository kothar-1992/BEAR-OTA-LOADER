...
// Note: p_data, and p_max are _pointers_ to a memory address holding the data. For a Drag widget, p_min and p_max are optional.
...
struct slider_state {
 ...
 ImTextureID subtab1IMG;

 ImTextureID sliderIMG;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(pos + ImVec2(220, 13), pos + ImVec2(220 + 295, 73 + 7));
    const ImRect total_bb(ImVec2(pos.x + 428, pos.y - 18), ImVec2(pos.x + 428 + 76, pos.y + 58 + 38));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &frame_bb, flags & ImGuiSliderFlags_NoInput ? 0 : ImGuiItemFlags_Inputable))
        return false;
    if (format == NULL)
        format = DataTypeGetInfo(data_type)->PrintFmt;
    const bool hovered = ItemHoverable(frame_bb, id);
    bool temp_input_is_active = (flags & ImGuiSliderFlags_NoInput) == 0 && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
        const bool input_requested_by_tabbing = (flags & ImGuiSliderFlags_NoInput) == 0 && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? c::tab_active : hovered ? c::tab_active : c::tab_active, g.IO.DeltaTime * 6.f);
    
    
    it_anim->second.hint_slider = ImLerp(it_anim->second.hint_slider, IsItemHovered() ? 1.f : 0.f, g.IO.DeltaTime * 35.f);
    GetWindowDrawList()->AddImageRounded(subtab1IMG, pos, ImVec2(pos.x + 529, pos.y + 93), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::tab_text_active), 0);
    GetWindowDrawList()->AddRectFilled(pos + ImVec2(240, 43), pos + ImVec2(210 + 295, 43 + 7), GetColorU32(c::slider_background), 7.5);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(20, 0), ImVec2(grab_bb.Max.x - 23, frame_bb.Max.y), ImColor(255, 0, 0, 0), 7.5);
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(20, 30), ImVec2(grab_bb.Max.x - 23, frame_bb.Max.y - 30), GetColorU32(c::slider_background_on), 7.5);
        float circle_start_x = grab_bb.Min.x - 50;
        float circle_end_x = grab_bb.Max.x + 20;
        float extended_circle_end_x = circle_end_x + 20;
        float circle_center_x = circle_start_x + (extended_circle_end_x - circle_start_x) / 2;
        GetWindowDrawList()->AddCircleFilled(ImVec2(circle_center_x, (frame_bb.Min.y + frame_bb.Max.y) / 2), 14, GetColorU32(c::slider_circle), 100);
        ImVec2 rect_size(55, 50);
       // ImVec2 rect_size(40, 35);
        ImVec2 rect_pos(circle_center_x - rect_size.x * 0.5f, frame_bb.Min.y - rect_size.y);
       if (IsItemActive())
        {
            GetWindowDrawList()->AddImageRounded(sliderIMG, rect_pos + ImVec2(0, 10), rect_pos + ImVec2(0, 10) + rect_size, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::tab_text_active), 0);
                
            char value_buf[64];
            DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
            ImVec2 text_size = CalcTextSize(value_buf);
            ImVec2 text_pos = rect_pos + ImVec2((rect_size.x - text_size.x) / 2, 15);
            GetWindowDrawList()->AddText(text_pos, GetColorU32(c::text_active), value_buf);
    ...
    if (data_type == ImGuiDataType_Float)
    {
        ...
        draw_label = value > min_value;
    }
    else if (data_type == ImGuiDataType_S32)
    {
        int value = *(int*)p_data;
        int min_value = *(int*)p_min;
        draw_label = value > min_value;
    }

        
        GetWindowDrawList()->AddText(ImVec2(pos.x + 25, pos.y + 32), ImColor(255,255,255), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (data_type == ImGuiDataType_S32)
    {
        ...
        GetWindowDrawList()->AddText(ImVec2(pos.x + 25, pos.y + 32), ImColor(0,0,0), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = ImClamp(window->Size.x - (window->Flags & ImGuiWindowFlags_Popup ? 24 : 12), 140.f, window->Size.x - (window->Flags & ImGuiWindowFlags_Popup ? 24 : 12));//175

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(13, 8), window->DC.CursorPos + ImVec2(window->Size.x - 13, label_size.y + 24));//24
   //     const ImRect frame_bb(window->DC.CursorPos + ImVec2(15, 8), window->DC.CursorPos + ImVec2(window->Size.x - 15, label_size.y + 24));//24
  //  const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
...
//const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min + ImVec2(0, 0), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha), label);

 //   ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(0, label_size.y + 5), frame_bb.Max - ImVec2(0, 4));
ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(20, label_size.y + 5), frame_bb.Max - ImVec2(20, 4));
    ...
    it_active_value->second = floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x));//100

    if (it_old_value->second != it_active_value->second)
    {
        if (it_old_value->second < it_active_value->second)
            it_old_value->second += (it_active_value->second - it_old_value->second) / 8 + 0.6f;//10 + 0.6f
        if (it_old_value->second > it_active_value->second)
            it_old_value->second -= ((it_old_value->second - it_active_value->second) / 8 + 0.6f);
    }

    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y ), ImColor(22, 22, 22, (int)(g.Style.Alpha * 255)), 8);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(30, 30, 30, (int)(g.Style.Alpha * 255)), 8);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y), ImColor(19, 148, 250, (int)(255 * g.Style.Alpha)), 8, ImDrawFlags_RoundCornersLeft);
    
    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Min.y + 3.5f), 8, ImColor(255, 255, 255, (int)(g.Style.Alpha * 255)));
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha  * it_old_value->second) - 2, slider_bb.Min.y - 2), ImVec2(slider_bb.Min.x + 2 + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y + 2), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha), 8);///6
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, (p_data), format);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x - 28, frame_bb.Min.y), ImColor(0 / 255.f, 0 / 255.f, 0 / 255.f, g.Style.Alpha), value_buf);
